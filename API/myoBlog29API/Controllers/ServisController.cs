...
namespace myoBlog29API.Controllers
{
    ...
    public class ServisController : ApiController
    {
        ...
        public SonucModel MakaleEkle(MakaleModel model)
        {
            ...
            {
                sonuc.islem = false;
                sonuc.mesaj = "Girilen Ders Başlığı Kayıtlıdır!";
                return sonuc;
            }
        ...
        public SonucModel MakaleSil(int makaleId)
        {
            ...
            {
                sonuc.islem = false;
                sonuc.mesaj = "Üzerinde Yorum Kayıtlı Ders Silinemez!";
                return sonuc;
            }
        ...
        [HttpPost]
        [Route("api/makalefotoguncelle")]
        public SonucModel MakaleFotoGuncelle(MakaleFotoModel model)
        {
            Makale makale = db.Makale.Where(s => s.MakaleId == model.MakaleId).SingleOrDefault();
            if (makale == null)
            {
                sonuc.islem = false;
            ...
            if (makale.Foto != "profil.png")
            {
                string yol = System.Web.Hosting.HostingEnvironment.MapPath("~/Dosyalar/Makale/" + makale.Foto);
                if (File.Exists(yol))
                {
                    File.Delete(yol);
                }
            ...
            string dosyaAdi = makale.MakaleId + model.FotoUzanti.Replace("image/", ".");
            using (var ms = new MemoryStream(imgBytes, 0, imgBytes.Length))
            {
                Image img = Image.FromStream(ms, true);
                img.Save(System.Web.Hosting.HostingEnvironment.MapPath("~/Dosyalar/Makale/" + dosyaAdi));
            }
